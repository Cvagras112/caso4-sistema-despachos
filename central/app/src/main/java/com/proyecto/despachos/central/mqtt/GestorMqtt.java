package com.proyecto.despachos.central.mqtt;
 ...
 *
 * Raíz de tópicos de zona: despachos/puerto-montt/{zona}/nuevos  (zona = centro | alerce | tepual)
 * Tópico de estado:        despachos/{idDespacho}/estado
 */
public class GestorMqtt {
    ...
    /**
     * Encamina un mensaje MQTT según su tópico.
    ...
    private void notificarEstadoActual() {
        Escuchador actual = escuchador;
        if (actual != null) {
            actual.alCambiarConexion(conectado, conectado ? "Conectado al broker" : "Desconectado");
        }
    }
